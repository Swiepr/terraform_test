...
resource "aws_lb_listener_rule" "jenkins" {
  ...
  condition {
    host_header {
      values = ["aws02-jenkins.busanit-lab.com"]
    }
  }
}
...
resource "aws_autoscaling_group" "example" {
  ...
  target_group_arns = [aws_lb_target_group.aws02-tg.arn]
...
resource "aws_autoscaling_attachment" "asg_attachment_target" {
  ...
  lb_target_group_arn = aws_lb_target_group.aws02-tg.arn
}
...
resource "aws_lb_listener_rule" "asg" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.aws02-tg.arn
  }

  condition {
    host_header {
      values = ["aws02-target.busanit-lab.com"]
    }
  }
}

// 대상그룹 - target group
resource "aws_lb_target_group" "aws02-tg" {
